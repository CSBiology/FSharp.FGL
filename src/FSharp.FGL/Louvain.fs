...
module Louvain =                   
    ...
    let (*inline *)private sumGroupBy (groupingF : 'T -> 'group) (valueF : 'T -> 'V) (input : ('T) []) =
    ...
    let private findWeightofConnectionToOldCommunity connectedCommunities originalCommunity     =   
        
        match (Array.tryFind (fun (community,weight) -> community=originalCommunity) connectedCommunities) with
            | Some x    -> (x|> snd)
            | None      -> 0.
         
    let louvain (g1:ArrayAdjacencyGraph<'Vertex,'Label,float>) = //(g:ArrayAdjacencyGraph<'Vertex,'Label,float>) =
            
        let vertices,vertices2 =
        ...
        let g = ArrayAdjacencyGraph(edges,vertices)
        let g2 = ArrayAdjacencyGraph(edges,vertices2)

        let louvainCycleInPlace (graph:ArrayAdjacencyGraph<'Vertex,'Label*int,float>)   =
               ...
               let communitySumtotalSumintern =
                   ...
                   for i=0 to graph.VertexCount-1 do
                       ...
                       let communityWeightTotalStart =  ki.[i]//(graph.WeightedDegree ((Array.sumBy(fun (s,t,w) -> w)),vertex))
                       let selfLoopsStart = selfLoops.[i] //vertex
               ...
               let rec loop nbOfMoves currentQuality improvement =
                   let build (shouldIBuild:bool) =
                       if not shouldIBuild then
                           ...
                           let (vertexToLabelMap,vertexNewLabel) =
                               let labelMap =
                                   graph.GetLabels()
                                   |> Array.map snd
                                   |> Array.distinct
                               ...
                               let vertexDict = System.Collections.Generic.Dictionary<'Vertex,int>()
                               for i in verti do
                           ...
                           for i in g.GetVertices() do
                               ...
                               let updateLabel     = vertexToLabelMap.[currentLabel] //Dictionary.getValue currentLabel 
        ...
        let rec louvainInPlace_ nbOfLoops (newG:ArrayAdjacencyGraph<'Vertex,'Label*int,float>) =
    ...
    let louvainRnd (g1:ArrayAdjacencyGraph<'Vertex,'Label,float>) = //(g:ArrayAdjacencyGraph<'Vertex,'Label,float>) =
